...
class DictSimilarityEvaluator2Config(BaseModel):
...
class DictSimilarityEvaluator2(PromptSetModifier):
    def __init__(self, config: DictSimilarityEvaluator2Config):
        self.config = config
    ...
    async def modify(self, prompt_set: PromptSet) -> PromptSet:
        ...
        def extract_dict(
            ...
            if reference_dict:
                ...
                for key in reference_dict.keys():
                    # if extracted value is int but reference value is float, it should be considered a match
                    if (
                        type(reference_dict[key]) is float
                        and type(extracted_dict[key]) is int
                    ):
                        extracted_dict[key] = float(extracted_dict[key])
        ...
        async def process_prompt_reference(prompt, reference):
            ...
            for message in messages:
                ...
                extracted_reference, extraction_error = extract_dict(reference.output)
                if extraction_error:
                    logger.error(
                ...
                logger.info(
                    f"Evaluated {self.queries_executed}/{total_queries} queries."
                )
