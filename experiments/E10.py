...
async def main(n, k):
    ...
    text_gradient_config = BasicTextGradientConfig(
        llm=llama31_70b,
        n_worst_performing_references=2,
        meta_prompt=gradient_meta_prompt,
    )
    text_gradient = BasicTextGradient(text_gradient_config)
...
if __name__ == "__main__":
    ...
    pset = results[20][0]
    pset_n = pset[:1] + pset[5:]
    bscore_traj = [
        max([p.mean_score for p in pset_n[: i + 1]]) for i in range(0, 102, 5)
    ]

    # save as json
    with open("e10_bscore_traj_se.json", "w") as f:
        json.dump(bscore_traj, f)
    ...
    # save as json
    with open("e10_bscore_traj_rb.json", "w") as f:
        json.dump(bscore_traj, f)
