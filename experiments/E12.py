...
from prompt_optimization.alternating_prompt_optimizer import (
    AlternatingPromptOptimizer,
)
...
async def main(n, k):
    ...
    text_gradient_config = AlternatingTextGradientConfig(
        llm_base_level=llama31_70b,
        llm_base_level_last_step=llama31_70b_eval,
        llm_meta_level=llama31_70b_eval,
    )
    text_gradient = AlternatingTextGradient(text_gradient_config)

    # Create TopKModifier for pruning
    top_k_modifier = TopKPromptPruner(k=3)

    # Create AlternatingPromptOptimizer for SummEval
    se_optimizer = AlternatingPromptOptimizer(
        training_metric=se_metric,
        prune_step=top_k_modifier,
        optimize_step=text_gradient,
        validation_metric=None,
    )

    # Create AlternatingPromptOptimizer for RewardBench
    rb_optimizer = AlternatingPromptOptimizer(
        training_metric=rb_metric,
        prune_step=top_k_modifier,
        optimize_step=text_gradient,
        validation_metric=None,
    )

    # Run optimization for SummEval
    prompt_set_se = await se_optimizer.run(
        prompt_set=[
            Prompt(prompt=p)
            for p in [se_instr1, se_instr2, se_instr3, se_instr4, se_instr5]
        ],
        meta_learner_steps=4,
        base_learner_steps=5,
    )

    # Run optimization for RewardBench
    prompt_set_rb = await rb_optimizer.run(
        prompt_set=[
            Prompt(prompt=p)
            for p in [rb_instr1, rb_instr2, rb_instr3, rb_instr4, rb_instr5]
        ],
        meta_learner_steps=4,
        base_learner_steps=5,
        n_best_validation=3,
    ...
    steps = [5, 10, 20]
    for step in steps:
        best_prompt_se = best_unvalidated_prompt(prompt_set_se[: step + 1])
        best_prompt_rb = best_unvalidated_prompt(prompt_set_rb[: step + 1])

        await asyncio.gather(se_val([best_prompt_se]), rb_val([best_prompt_rb]))
