...
gradient_meta_prompt = """A model used an instruction to solve a task.
...
if __name__ == "__main__":
    ...
    bscore_traj = [max([p.mean_score for p in pset_n[: i + 1]]) for i in range(22)]

    # save as json
    with open("e8a_bscore_traj_se.json", "w") as f:
    ...
    # save as json
    with open("e8a_bscore_traj_rb.json", "w") as f:
        json.dump(bscore_traj, f)
