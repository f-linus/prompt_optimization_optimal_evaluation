...
from prompt_optimization.operators.crossover import Crossover
...
from prompt_optimization.utils.plot import plot_prompt_embeddings_pca
...
async def main(n, k):
    ...
    crossover_op = Crossover(llm=llama31_70b)
    mutation_op = Mutation(llm=llama31_70b)
    ...
    for step in tqdm(range(n)):
        ...
        # mutation
        best_se = sorted(prompt_set_se, key=lambda x: x.mean_score, reverse=True)[:2]
        best_rb = sorted(prompt_set_rb, key=lambda x: x.mean_score, reverse=True)[:2]
        ...
        child_se, child_rb = await asyncio.gather(
            crossover_op(best_se), crossover_op(best_rb)
        )

        # additionally mutate children
        new_prompts_se, new_prompts_rb = await asyncio.gather(
            mutation_op(child_se), mutation_op(child_rb)
        )
    ...
    fig, ax = plot_prompt_embeddings_pca(prompt_set_se[:10])
    fig.savefig("E7_se.png", dpi=300)
    fig, ax = plot_prompt_embeddings_pca(prompt_set_rb[:10])
    fig.savefig("E7_rb.png", dpi=300)
